				...
				<table class="adminlist">
					<thead>
						<tr>
							...
							<th width="20%" nowrap="nowrap">LDAP Group</th>
					...
					for ($i=0, $n=count( $rows ); $i < $n; $i++) {
						...
						$sync = (isset($lists[$row->id]['insync']) && $lists[$row->id]['insync']) ? true: false ;						
					...
					?>
						<tr class="<?php echo "row$k"; ?>">
							...
							<td align="center"><?php echo $row->username;?></td>
							 <td><?php if(isset($lists['ugroups'][$row->id])){foreach($lists['ugroups'][$row->id] as $groups) {echo '<ul>' . $groups['gname'] . '</ul>'; } }else {echo "<ul>No group set.</ul>" ; } ?></td>
						
							<td><?php if(isset($lists['ugroups'][$row->id])){foreach($lists['ugroups'][$row->id] as $groups) {echo '<ul>' . $groups['dn'] . '</ul>'; } }else {echo "<ul>No group set.</ul>" ; } ?></td>
							
							<td align="center"><?php echo isset($lists[$row->id]['dn']) ? $lists[$row->id]['dn'] : JText::_('No DN Set'); $lastid= $row->id; ?></td>							
						</tr>
					<?php
					$k = 1 - $k;
					}
